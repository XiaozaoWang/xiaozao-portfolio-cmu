    ...
    import("p5").then((p5Module) => {
      ...
      const sketch = (p: any) => {
        ...
        p.setup = () => {
          ...
          prism = new Prism(p.mouseX, p.mouseY, (400 * 1) / 10);
          for (let i = 0; i < 6; i++) {
            beams.push(new Beam(0, p.random(400), p.random(2 * p.PI), 30));
            beams.push(
              new Beam(
            ...
            beams.push(
              new Beam(
                p.random(container?.offsetWidth || 800),
                0,
                p.random(2 * p.PI),
                30
              )
    ...
    <div className="mt-16">
      <div className="mb-6">
        ...
        <p className="text-gray-600 dark:text-gray-400 text-sm">
          Interactive p5.js sketch showing light refraction through a prism.
          Move your mouse to control the prism, click or use arrow keys to
          rotate. Press 'R' to toggle rainbow mode.
        </p>
      </div>
      <div
        ref={sketchRef}
        className="w-full h-[400px] bg-black rounded-lg overflow-hidden shadow-lg border border-gray-200 dark:border-gray-700"
      />
    </div>
  );
}
